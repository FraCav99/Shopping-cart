...
const Cart = ({ cartItems, setCartItems }) => {
  ...
  const incrementQuantity = ev => {
    const elementId = ev.target.id;
    const updatedPrice = cartItems.map(item =>
      item.id === +elementId ? { ...item, quantity: item.quantity + 1 } : item
    );
              ...
              <div className="product-item" key={cartItem.id}>
                ...
                <div className="product-quantity column">
                  <div className="quantity-setter">
                    <div>
                      <button id={cartItem.id} onClick={decrementQuantity}>
                        -
                      </button>
                    </div>
                    <span>{cartItem.quantity}</span>
                    <div>
                      <button id={cartItem.id} onClick={incrementQuantity}>
                        +
                      </button>
                    </div>
                  </div>
                ...
                <div className="product-subtotal column">
                  ...
                  <button onClick={deleteItem} id={cartItem.id}>
                    X
                  </button>
                </div>
              </div>
